with GNATCOM.Interface;

with GNATCOM.Errors;

package body winword.TabStops_Interface is

   procedure Initialize (This : in out TabStops_Type) is
   begin
      Set_IID (This, IID_TabStops);
   end Initialize;

   function Pointer (This : TabStops_Type)
     return Pointer_To_TabStops
   is
   begin
      return To_Pointer_To_TabStops (Address (This));
   end Pointer;

   procedure Attach (This    : in out TabStops_Type;
                     Pointer : in     Pointer_To_TabStops)
   is
   begin
      Attach (This, GNATCOM.Interface.To_Pointer_To_IUnknown
              (Pointer.all'Address));
   end Attach;

   function Getu_NewEnum
     (This : TabStops_Type)
     return GNATCOM.Types.Pointer_To_IUnknown
   is
       RetVal : aliased GNATCOM.Types.Pointer_To_IUnknown;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Getu_NewEnum
         (Pointer (This),
          RetVal'Unchecked_Access));

      return RetVal;
   end Getu_NewEnum;

   function Get_Count
     (This : TabStops_Type)
     return Interfaces.C.long
   is
       RetVal : aliased Interfaces.C.long;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Get_Count
         (Pointer (This),
          RetVal'Unchecked_Access));

      return RetVal;
   end Get_Count;

   function Get_Application
     (This : TabStops_Type)
     return Pointer_To_uApplication
   is
       RetVal : aliased Pointer_To_uApplication;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Get_Application
         (Pointer (This),
          RetVal'Unchecked_Access));

      return RetVal;
   end Get_Application;

   function Get_Creator
     (This : TabStops_Type)
     return Interfaces.C.long
   is
       RetVal : aliased Interfaces.C.long;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Get_Creator
         (Pointer (This),
          RetVal'Unchecked_Access));

      return RetVal;
   end Get_Creator;

   function Get_Parent
     (This : TabStops_Type)
     return GNATCOM.Types.Pointer_To_IDispatch
   is
       RetVal : aliased GNATCOM.Types.Pointer_To_IDispatch;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Get_Parent
         (Pointer (This),
          RetVal'Unchecked_Access));

      return RetVal;
   end Get_Parent;

   function Item
     (This  : TabStops_Type;
      Index : access GNATCOM.Types.VARIANT :=
        GNATCOM.Types.PVARIANT_MISSING)
     return Pointer_To_TabStop
   is
       RetVal : aliased Pointer_To_TabStop;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Item
         (Pointer (This),
          Index,
          RetVal'Unchecked_Access));

      return RetVal;
   end Item;

   function Add
     (This      : TabStops_Type;
      Position  : Interfaces.C.C_float;
      Alignment : access GNATCOM.Types.VARIANT :=
        GNATCOM.Types.PVARIANT_MISSING;
      Leader    : access GNATCOM.Types.VARIANT :=
        GNATCOM.Types.PVARIANT_MISSING)
     return Pointer_To_TabStop
   is
       RetVal : aliased Pointer_To_TabStop;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Add
         (Pointer (This),
          Position,
          Alignment,
          Leader,
          RetVal'Unchecked_Access));

      return RetVal;
   end Add;

   procedure ClearAll
     (This : TabStops_Type)
   is
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.ClearAll
         (Pointer (This)));

   end ClearAll;

   function Before
     (This     : TabStops_Type;
      Position : Interfaces.C.C_float)
     return Pointer_To_TabStop
   is
       RetVal : aliased Pointer_To_TabStop;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.Before
         (Pointer (This),
          Position,
          RetVal'Unchecked_Access));

      return RetVal;
   end Before;

   function After
     (This     : TabStops_Type;
      Position : Interfaces.C.C_float)
     return Pointer_To_TabStop
   is
       RetVal : aliased Pointer_To_TabStop;
   begin
      GNATCOM.Errors.Error_Check
        (Pointer (This).Vtbl.After
         (Pointer (This),
          Position,
          RetVal'Unchecked_Access));

      return RetVal;
   end After;

end winword.TabStops_Interface;

